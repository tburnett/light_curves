...
__all__ = ['config', 'files', 'check_source', 'add_weights']
...
# Cell
def check_source(files, source):

    weight_file =  os.path.join(files.weights, source.filename+'_weights.pkl')
    if not os.path.exists(weight_file):
        available = np.array(list(map(lambda p: p.name[:p.name.find('_weights')],
                          files.weights.glob('*_weights.pkl'))))
        print(f'Source {source.name} not found in list of weight files. Available:\n{available}',
             file = sys.stderr)
        raise Exception()
    return weight_file
...
def add_weights(xconfig, xfiles, photon_data, source):
    ...
    weight_file =  check_source(files, source)
